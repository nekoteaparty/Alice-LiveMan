package site.alicemononobe.liveman.mediaproxy.proxytask;
...
public class M3u8MediaProxyTask extends MediaProxyTask {
                ...
                for (String m3u8Line : m3u8Lines) {
                    if (!m3u8Line.startsWith("#")) {
                        ...
                        if (currentSeqIndex > lastSeqIndex) {
                            ...
                            String queueData = m3u8Line + "@" + getTempPath() + "/m3u8/" + getVideoId() + "/" + currentSeqIndex + ".ts";
    ...
    private void createM3U8File() throws IOException {
        ...
        File m3u8Path = new File(getTempPath() + "/m3u8/" + getVideoId() + "/");
        m3u8Path.mkdirs();
